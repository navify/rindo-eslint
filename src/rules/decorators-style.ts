...
import { decoratorName, getDecorator, rindoComponentContext, rindoDecorators } from '../utils';
...
const rule: Rule.RuleModule = {
  meta: {
    docs: {
      description: 'This rule catches Rindo Decorators not used in consistent style.',
  ...
  create(context): Rule.RuleListener {
    ...
    function getStyle(node: any) {
      if (!rindo.isComponent() || !options || !Object.keys(options).length) {
        return;
      }
      const decorators: any[] = getDecorator(node);
      decorators.filter((dec) => rindoDecorators.includes(decoratorName(dec))).forEach(checkStyle);
    }
