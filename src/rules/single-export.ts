...
const rule: Rule.RuleModule = {
  meta: {
    docs: {
      description: 'This rule catches modules that expose more than just the Rindo Component itself.',
      ...
      'ClassDeclaration': (node: any) => {
        ...
        if (component) {
          ...
          nonTypeExports.forEach(symbol => {
            ...
            context.report({
              node: errorNode,
              message: `To allow efficient bundling, modules using @Component() can only have a single export which is the component class itself. Any other exports should be moved to a separate file. For further information check out: https://rindojs.web.app/guide/module-bundling`
            });
          });
        }
      }
